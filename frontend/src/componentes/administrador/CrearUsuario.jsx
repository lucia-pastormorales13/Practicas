...
export default function CrearUsuario() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await api.post("/admin/crear-usuario", dataForm);
            if (response.status === 201) {
                ...
                setDataForm({ nombre: "", apellidos: "", correo: "", contrasenia: "", rol: "" });
                navigate("/administrador-dashboard");
            } else {
                const errorMsg = response.data?.message || "Error al agregar el usuario.";
                setMessage(errorMsg);
            ...
            <div className="bg-white rounded-2xl max-w-md w-full mx-auto p-6 shadow-xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        <button type="submit" className="flex-1 py-3 rounded-xl bg-linear-to-r from-blue-500 to-purple-600 text-white hover:shadow-lg transition-all">Crear Usuario</button>
                    </div>
